...
def fetch_medal_tally(df, year, country):
    medal_df = df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal','Gold', 'Silver', 'Bronze'])
    flag = 0
...
def country_year_list(df):
    ...
    country = df['region'].drop_duplicates().tolist()
    # Ensure all elements in the country list are strings
...
def country_event_heapmap(df,country):
    temp_df = df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)
    new_df = temp_df[temp_df['region'] == country]
